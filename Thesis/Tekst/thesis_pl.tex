...
Narzędzie może być wykorzystywane zarówno przez lekarzy w celach diagniostycznych, jak i~inżynierów
...
The following document describes bachelor's thesis on~"\title". The goal of~authors'
is~to~create a~digital tool that allows to~manually draw and semi-automatically generate
contours of anatomical structures and lesions on~medical images.
Tool can be used by medical doctors with diagnostic purposes and by medical engineers
with scientific purposes. The topics discussed include: discussion
of~DICOM standard, analysis of~state-of-the-art systems and tools used to~viewing
...
Semi-automatic module is based on the algorithm developed for the~purpose of~generating contours, that uses Canny's
Operator and the algorithm of~searching the shortest path in~the graph.
...
były konsultowane ze %z~lekarzami i~studentami ostatniego roku medycyny z Warszawskiego Uniwersytetu Medycznego.
...
\begin{center}
	...
	Wygenerowany w~ten sposób obrys półautomatyczny (e)}
	\label{fig:174}
\end{center}
\end{figure}
...
\begin{center}
	...
	na wskutek wgniecenia kości ramiennej przez krawędź panewki łopatki i wyrostka kruczego łopatki.
...
\bibitem{GK1} Kotowski P. Wykłady z~przedmiotu Grafika Komputerowa~1~\emph{Politechnika Warszawska, Wydział Matematyki i~Nauk Informacyjnych} Materiały dostepne na~stronie: http://mini.pw.edu.pl/~kotowski/gk/gk.html [Dostęp 5~kwietnia 2019]
