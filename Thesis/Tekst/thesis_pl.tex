...
Podobnie jak Orthanc, SonicDicom posiada on wbudowany interfejs do przeglądania
zapisanych w systemie badań, który nie umożliwia wykonywania obrysów na badaniach.

\begin{figure}[tb!]
	\center
	\includegraphics[width=0.55\textwidth]{SoniCDICOMViewer}
	\caption{Podgląd obrazu DICOM przy użyciu interfejsu SonicDICOM \cite{SonicDICOM img}}
    	\label{fig:Orthanc-preview}
\end{figure}

System SonicDICOM jest rozwiązaniem komercyjnym i wykorzystanie go wymaga wykupienia
licencji w celu korzystania z zawarnych w nim funkcjonalności.
...
\begin{figure}[tb!]
	\center
	\includegraphics[width=0.55\textwidth]{architektura}
	\caption{Architektura proponowanego rozwiązania}
    	\label{fig:Orthanc-preview}
\end{figure}
...
Specyfikacja wymagań została opracowana na podstaiwe danych dotyczących wymagań stawianych
przed tworzonym systemem. Zostały one przedstawione w~rozdziałach 3.1.1. - 3.1.3. Wymagania
były konsultowane z~lekarzami i~ze~studentami Warszawskiego Uniwersytetu Medycznego.
...
związanych z wykonananym obrysem statystyk. Praca
...
Wymagania funkcjonalne zostały przygotowane na podstawie konsultacji z~lekarzami
i~studentami kierunków medycznych dla tworzonego systemu. Przedstawiono
...
obszarów ujętych w tabeli należą: użyteczność, niezawodność oraz wydajność.
...
identyfikator obrysu, nazwa obrysu oraz informacja, czy obraz został wygenerowany
...
Moduł służy do samodzielnego wykonywania obrysów przy użyciu myszy albo tabletu
...
gdy moduł obrysów półautomatycznych nie daje satysfakcjonujących użytkownika rezultatów.
...
pozycji punktów na obrazie, przez które użytkownik przesuwał kursor oraz kolor obrysu.
...
pole obrysu, środek ciężkości obrysu, histogram obrazu zawartego w obrysie oraz
liczba pikseli wewnątrz obrysu.

W związku z~tym, że obrazy wyświetlane w~interfejsie są w~innej rozdzielczości
niż rzeczywisty rozmiar obrazu (spowodownae jest to potrzebą zapewnienia, że obrys
będzie wyświetlany na ekranie w całości jak również, że obrazy o małej rozdzielczości
będą dostatecznie czytelne dla użytkownika), obrysy przed zapisaniem zostają przeskalowane do
...
użytkownika punktów przez odwrotność współczynnika skalowania obrazu.
 W~przypadku obrazów o~małych rozdzielczościach może
spowodować to~utratę dokładności obrysu, a~w~przypadku obrysów bardzo małych
obszarów zdegradowanie obrysu do~jednego piksela \ref{fig:manual-0px}. Tego typu straty mogą wpływać
na poprawność obliczanych statystyk jak również błędne wyświetlanie zapisanych
obrysów w~interfejsie użytkownika.

\begin{figure}[h]
	\center
	\includegraphics[width=0.7\textwidth]{manualny-0px}
